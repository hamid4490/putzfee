...
async def _send_ntfy(topics: List[str], title: str, body: str, data: Optional[dict]):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        for topic in topics:
            ...
            try:
                await client.post(url, headers=headers, content=(body or ""))
            except Exception:
                pass
...
async def confirm_slot(order_id: int, body: ConfirmSlotRequest):
    ...
    sel_slot = ScheduleSlotTable.__table__.select().where((ScheduleSlotTable.request_id == order_id) & (ScheduleSlotTable.slot_start == chosen_start) & (ScheduleSlotTable.status == "PROPOSED"))
