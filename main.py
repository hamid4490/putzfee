...
async def propose_slots(order_id: int, body: ProposedSlotsRequest, request: Request):
    ...
    slots = sorted(list(set(body.slots)))[:2]
    if not slots:
...
@app.post("/order/{order_id}/accept_preferred_slot")
async def accept_preferred_slot(order_id: int, body: ConfirmSlotRequest, request: Request):
    require_admin(request)
    provider = get_admin_provider_phone(request)
    ...
    if st in FINAL_ORDER_STATUSES or req["execution_start"]:
        raise HTTPException(status_code=409, detail="cannot accept preferred slot")

    pref_raw = req["preferred_slots"] or []
    if not pref_raw:
        raise HTTPException(status_code=400, detail="no preferred slots")
    ...
    for x in pref_raw:
        try:
            pref_minutes.add(int(parse_iso_utc(x).timestamp() // 60))
        except Exception:
            pass

    if pref_minutes and int(slot_dt.timestamp() // 60) not in pref_minutes:
        raise HTTPException(status_code=400, detail="slot not in preferred")

    if not await provider_is_free(provider, slot_dt, end_dt, exclude_order_id=int(order_id)):
        raise HTTPException(status_code=409, detail="slot overlap")
    ...
    try:
        ...
        logger.error(f"notify_user(accept_preferred) failed: {e}")

    return unified_response("ok", "PREFERRED_ACCEPTED", "preferred slot accepted", {
        "start": slot_dt.isoformat(),
        "end": end_dt.isoformat(),
    })

@app.post("/order/{order_id}/reject_preferred_slots")
async def reject_preferred_slots(order_id: int, request: Request):
    require_admin(request)
    ...
    st = canon_status(str(req["status"] or ""))
    if st in FINAL_ORDER_STATUSES or req["execution_start"]:
        raise HTTPException(status_code=409, detail="cannot reject preferred slots")

    async with database.transaction():
        await database.execute(
            RequestTable.__table__.update().where(
                RequestTable.id == int(order_id)
            ).values(preferred_slots=[])
        )
    ...
    try:
        await notify_user(
            ...
            body="",
            data=push_event_data(
                event="preferred_slots_rejected",
                order_id=int(order_id),
                status=st,
                service_type=str(req["service_type"] or ""),
                user_phone=str(req["user_phone"] or ""),
        ...
        logger.error(f"notify_user(reject_preferred) failed: {e}")

    return unified_response("ok", "PREFERRED_REJECTED", "preferred slots rejected", {})
