...
# FastAPI server (orders + hourly scheduling + DB notifications + FCM push + AdminKey)  # توضیح=سرور با کلید ادمین
...
class RequestTable(Base):  # مدل=سفارش‌ها
    ...
    scheduled_start = Column(DateTime(timezone=True), nullable=True)  # شروع قطعی
...
async def notify_user(phone: str, title: str, body: str, data: Optional[dict] = None):  # تابع=ثبت اعلان
...
async def create_order(order: OrderRequest):  # تابع=ایجاد سفارش
    ...
    try:  # try
        await send_push_to_managers("درخواست جدید", "درخواست جدید ثبت شد.", {"type": "new_request", "order_id": str(new_id)})  # پوش
    except Exception:  # خطا
        pass  # نادیده
...
async def cancel_order(cancel: CancelRequest):  # تابع=لغو
    upd = (RequestTable.__table__.update().where((RequestTable.user_phone == cancel.user_phone) & (RequestTable.service_type == cancel.service_type) & (RequestTable.status.in_(["NEW", "WAITING", "ASSIGNED", "IN_PROGRESS"]))).values(status="CANCELED", scheduled_start=None).returning(RequestTable.id))  # آپدیت
            ...
            for r in rows:  # حلقه
                ...
                if mapping and "id" in mapping:  # کلید id
                    ...
                    try:
                        ...
                        oid = None  # None
                await send_push_to_managers("لغو درخواست", "کاربر سفارش را لغو کرد.", {"type": "order_canceled", "order_id": str(oid) if oid is not None else ""})  # پوش
        except Exception:  # خطا
            pass  # نادیده
...
async def get_free_hours(provider_phone: str, date: str, work_start: int = 8, work_end: int = 20, limit: int = 24):  # تابع=ساعات آزاد
    ...
    while cur + timedelta(hours=1) <= day_end and len(results) < limit:  # حلقه تولید اسلات
        ...
        if provider.lower() == "any" or await provider_is_free(provider, s, e):  # آزاد یا any
...
async def get_busy_slots(provider_phone: str, date: str, exclude_order_id: Optional[int] = None):  # تابع=busy
    ...
    sel_sched = ScheduleSlotTable.__table__.select().where((ScheduleSlotTable.slot_start >= day_start) & (ScheduleSlotTable.slot_start < day_end) & (ScheduleSlotTable.status.in_(["PROPOSED", "ACCEPTED"])))  # انتخاب
    if provider_phone.strip().lower() != "any":  # فیلتر provider
        sel_sched = sel_sched.where(ScheduleSlotTable.provider_phone == provider_phone)  # شرط provider
    ...
    sel_app = AppointmentTable.__table__.select().where((AppointmentTable.start_time >= day_start) & (AppointmentTable.start_time < day_end) & (AppointmentTable.status == "BOOKED"))  # رزرو قطعی
    if provider_phone.strip().lower() != "any":  # فیلتر provider
        sel_app = sel_app.where(AppointmentTable.provider_phone == provider_phone)  # شرط
...
async def propose_slots(order_id: int, body: ProposedSlotsRequest, request: Request):  # تابع=ثبت اسلات
    ...
    if accepted:  # وجود اسلات
        ...
        try:  # try
            await notify_user(req["user_phone"], "زمان‌بندی", "لطفاً یکی از زمان‌های پیشنهادی را انتخاب کنید.", data={"order_id": order_id, "slots": accepted})  # اعلان
        except Exception:  # خطا
            pass  # نادیده
...
async def confirm_slot(order_id: int, body: ConfirmSlotRequest):  # تابع=تأیید
    ...
    try:  # try
        await send_push_to_managers("تأیید زمان", "کاربر زمان را تأیید کرد.", {"type": "time_confirm", "order_id": str(order_id)})  # پوش
    except Exception:  # خطا
        pass  # نادیده
...
async def reject_all_and_cancel(order_id: int):  # تابع=رد+کنسل
    ...
    try:  # try
        await send_push_to_managers("لغو درخواست", "کاربر سفارش را لغو کرد.", {"type": "order_canceled", "order_id": str(order_id)})  # پوش
    except Exception:  # خطا
        pass  # نادیده
...
@app.get("/debug/users")  # مسیر=دیباگ کاربران (باقی گذاشته شده برای سازگاری)
